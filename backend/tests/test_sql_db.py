...
@pytest.mark.asyncio
async def test_sql_valid_query():
    ...
    """
    result = await sql_db_tool.ainvoke({"query": "SELECT name, salary FROM employees WHERE name='Alice'"})
...
@pytest.mark.asyncio
async def test_sql_valid_query_bob():
    """Bob should always be present in the mock database."""
    result = await sql_db_tool.ainvoke({"query": "SELECT name, salary FROM employees"})
...
async def test_sql_syntax_error():
    ...
    result = await sql_db_tool.ainvoke({"query": "SELECT * FROM nonexistent_table_xyz"})
